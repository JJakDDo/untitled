...
const manualBattle = async (req, res) => {
  ...
  if (mh <= 0) {
    ...
    for (let i = 0; i < drops.length; i++) {
      const item = await Item.findById(drops[i]);
      if (item) {
        const itemAmount = Math.floor(Math.random() * (item.maxDrop + 1));
        if (itemAmount > 0) {
          logs.push(`${item.name} 을/를 ${itemAmount}개 획득!`);
          dropped.push({
          ...
          const indexOfItem = inventory.findIndex(
            (obj) => obj._id === item._id.toString()
          );
          if (indexOfItem >= 0) {
            // 아이템의 수량을 증가
            inventory[indexOfItem].amount += itemAmount;
          } else {
            inventory.push({
              _id: item._id.toString(),
              name: item.name,
